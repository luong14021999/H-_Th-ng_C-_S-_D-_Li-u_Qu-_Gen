  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100">
          ...
          {!readOnly && (
            <button
              ...
              className="min-w-[40px] min-h-[40px] p-2 rounded hover:bg-gray-200 active:bg-gray-300 text-gray-600 transition-colors flex items-center justify-center touch-manipulation"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 px-3 py-2">
          <div className="flex items-center gap-3 min-w-0">
            <span className="text-sm text-blue-700 font-medium w-16 shrink-0">Vĩ độ :</span>
            {editing ? (
              <input
                type="number"
                inputMode="decimal"
          ...
          <div className="flex items-center gap-3 min-w-0">
            <span className="text-sm text-blue-700 font-medium w-16 shrink-0">Kinh độ :</span>
            {editing ? (
              <input
                type="number"
                inputMode="decimal"
      ...
      {/* Map — shorter on phones so the save button stays in view without scrolling */}
      <div
        ref={mapHostRef}
        className="w-full rounded-lg border border-gray-200 overflow-hidden h-[45vh] sm:h-[min(60vh,560px)] touch-manipulation"
      />
      ...
      {!readOnly && (
        <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2 sm:gap-3">
          {saveError && (
            <span className="text-xs text-red-600 bg-red-50 border border-red-200 rounded px-2 py-1.5 sm:py-1 text-center sm:text-left">{saveError}</span>
          )}
          <button
            type="button"
            onClick={handleSaveLocation}
            disabled={!dirty || saving}
            className="inline-flex items-center justify-center gap-2 px-4 py-3 sm:py-2 text-sm font-semibold rounded-lg bg-green-700 text-white hover:bg-green-600 active:bg-green-800 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors touch-manipulation w-full sm:w-auto"
          >
      ...
      {savedOk && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl px-6 sm:px-10 py-6 sm:py-8 flex flex-col items-center gap-3 sm:gap-4 max-w-sm w-full">
            ...
            <div className="text-center min-w-0 w-full">
              <p className="text-base sm:text-lg font-bold text-gray-800">Lưu vị trí thành công!</p>
