...
const Row = ({ label, value, onChange, rows, bold }: {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 sm:gap-3 items-start py-2 border-b border-gray-100">
    <label className={`text-sm pt-1.5 font-semibold ${bold ? 'text-gray-900' : 'text-gray-900'}`}>{label}</label>
...
const SubLabel = ({ children }: { children: React.ReactNode }) => (
  <p className="text-xs font-semibold text-gray-800 italic mt-2 mb-1">{children}</p>
);
