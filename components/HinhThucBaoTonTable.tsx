...
export default function HinhThucBaoTonTable() {
  ...
  const openEdit = (h: HinhThucBaoTon) => {
    setEditItem(h);
    setEditForm({ ten: h.ten, ma: String(h.ma), phuong_thuc_id: h.phuong_thuc_id, nhom: [...h.nhom] });
  };
  ...
  const resetEdit = () => {
    if (!editItem) return;
    setEditForm({ ten: editItem.ten, ma: String(editItem.ma), phuong_thuc_id: editItem.phuong_thuc_id, nhom: [...editItem.nhom] });
  };
        ...
        <FilterSection title="Trạng thái">
          {(["all","kich_hoat","an"] as const).map((v) => (
            <label key={v} className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="ht_status" checked={statusFilter===v} onChange={() => setStatusFilter(v)} className="accent-blue-600" />
              <span className="text-sm text-gray-700">{{ all:"Tất cả", kich_hoat:"Kích hoạt", an:"Ẩn" }[v]}</span>
            </label>
          ))}
        </FilterSection>
        <FilterSection title="Phương thức bảo tồn">
          <input type="text" placeholder="Tìm kiếm theo phương thức" value={phuongThucSearch}
            onChange={(e) => setPhuongThucSearch(e.target.value)}
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div>
                ...
                <select value={editForm.phuong_thuc_id} onChange={(e) => setEditForm(f => ({ ...f, phuong_thuc_id: e.target.value }))}
                  ...
                  {PHUONG_THUC_BAO_TON.filter(p => p.trang_thai === "kich_hoat").map(p => (
