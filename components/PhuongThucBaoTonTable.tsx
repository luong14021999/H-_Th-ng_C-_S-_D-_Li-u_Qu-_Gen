...
export default function PhuongThucBaoTonTable() {
  ...
  const openEdit = (p: PhuongThucBaoTon) => {
    setEditItem(p);
    setEditForm({ ten: p.ten, ma: p.ma });
  };
  ...
  const resetEdit = () => {
    if (!editItem) return;
    setEditForm({ ten: editItem.ten, ma: editItem.ma });
  };
    ...
    <div className="flex flex-1 overflow-hidden bg-gray-50" onClick={() => setStatusOpenId(null)}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-4 bg-white border-b border-gray-200">
          <h1 className="text-xl font-semibold text-gray-800 shrink-0">Phương thức bảo tồn</h1>
          <input type="text" placeholder="Tìm kiếm theo tên phương thức" value={search}
            ...
            <tbody>
              {filtered.map((p, i) => (
                <tr key={p.id} className="border-b border-gray-200 hover:bg-blue-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-800">{p.ten}</td>
                  <td className="px-4 py-3 text-gray-600">{p.ma}</td>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    <span className="text-red-500 mr-1">*</span>Tên phương thức
                  </label>
...
function Pagination({ total }: { total: number }) {
  return (
    <div className="flex items-center gap-1 mt-4">
      ...
      <button disabled className="px-2 py-1 text-gray-400 disabled:opacity-30">{">"}</button>
      <span className="ml-2 text-xs text-gray-400">{total} kết quả</span>
    </div>
  );
}
